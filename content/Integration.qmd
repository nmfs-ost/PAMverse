...
A primary goal of recent PAMverse developments is to provide a streamlined way for passive acoustic researchers to integrate data from multiple data streams into standardized, informative data products that can be used as a decision support tool. PAMverse includes multiple built in functionality to integrate relevant environmental data with acoustic detection or soundscape data as well as directly integrate soundscape data with acoustic detection information. The examples below are the initial set of figures that are currently available, if there is a figure of interest that could be repeatedly used for your research or by others, please reach to Taiki to explore incorporating it into the package. 
